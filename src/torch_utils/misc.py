...
def copy_params_and_buffers(src_module, dst_module, require_all=False):
    ...
    for name, tensor in named_params_and_buffers(dst_module):
        ...
        if name in src_tensors:
            if (name == 'b256.fromrgb.weight') and (tensor.shape != src_tensors[name].shape):
                mix_tensor = torch.cat((
                    src_tensors[name], tensor[:, 3:4, ...].to(src_tensors[name].device)
                ), dim=1)
                src_tensors[name] = mix_tensor
                print(f'Warning: mixing tensors when copying {name}: {tensor.shape} != {src_tensors[name].shape}', flush=True)
            try:
