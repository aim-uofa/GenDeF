...
from training.networks_deform_synlayer import SynthesisLayer

# #----------------------------------------------------------------------------

# @persistence.persistent_class
# class ToRGBLayer(torch.nn.Module):
#     def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, cfg={}):
#         super().__init__()
#         self.cfg = cfg
...
#         if self.is_last:  # NOTE follow deformable detr
#             print("is_last, weight with xavier_uniform_ init")
#             torch.nn.init.xavier_uniform_(self.weight, gain=1.0)
#         # TODO: zero init as an option
...
#                 conv_clamp=conv_clamp, channels_last=self.channels_last, is_last=is_last, cfg=cfg)
#             self.num_torgb += 1
...
class SynthesisNetwork3v(SynthesisNetwork):
    ...
    def forward(self, ws, t, c, motion_z=None, coords=None, feat_canonical_dict=None, **layer_kwargs):
        ...
        assert 'L13_256_128' in feat_canonical_dict
        canonical_feat = feat_canonical_dict['L13_256_128']  # (b, 128, 256, 256)
        canonical_feat = canonical_feat.repeat_interleave(t.shape[1], dim=0)  # (bt, 128, 256, 256)
        ...
        for name, w in zip(self.layer_names, ws[1:]):
            ...
            if self.cfg.low_rank is not None and self.cfg.low_rank > 0:
                x = getattr(self, name)(x, w, t=motion_v, canonical_feat=canonical_feat, **layer_kwargs)
            else:
                x = getattr(self, name)(x, w, canonical_feat=canonical_feat, **layer_kwargs)
            cnt += 1
