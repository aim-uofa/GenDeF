...
def modulated_conv2d(
    ...
    if not fused_modconv:
        ...
        x = conv2d_resample.conv2d_resample(x=x, w=weight.to(x.dtype), f=resample_filter, up=up, down=down, padding=padding, flip_weight=flip_weight, offsets=offsets, offsets_mask=offsets_mask)
    ...
    x = conv2d_resample.conv2d_resample(x=x, w=w.to(x.dtype), f=resample_filter, up=up, down=down, padding=padding, groups=batch_size, flip_weight=flip_weight, offsets=offsets, offsets_mask=offsets_mask)
...
class SynthesisLayer(torch.nn.Module):
    def __init__(self,
        ...
        if self.cfg.canonical_cond == "concat":
            ...
            cond_feat_dim = int(self.cfg.canonical_feat.split("_")[-1])
            self.canonical_feat_proj = None
            if cond_feat_dim != self.cfg.canonical_cond_dim:
                print("canonical_feat_dim=%d, canonical_cond_dim=%d" % (cond_feat_dim, self.cfg.canonical_cond_dim))
                self.canonical_feat_proj = FullyConnectedLayer(cond_feat_dim, self.cfg.canonical_cond_dim, bias_init=1)

        # for dcn
        self.kernel_size = kernel_size
        self.up = up
        self.dcn = self.cfg.deform_dcn and getattr(self.cfg, 'deform_dcn_syn', True) and (resolution >= self.cfg.deform_dcn_min_res and resolution <= self.cfg.deform_dcn_max_res) and (self.up == 1)  # NOTE: donot support up != 1 (transpose dcn)
        self.dcn_lr_mult = self.cfg.dcn_lr_mult
        dcn_affine_out_channel = affine_out_channels if getattr(self.cfg, 'deform_dcn_syn_concat', True) else in_channels
        if self.dcn:
            print("resolution=%d, use dcn" % resolution)
            dcn_out_channel = 3 * kernel_size ** 2
            self.dcn_affine = FullyConnectedLayer(w_dim, dcn_affine_out_channel, bias_init=1)
            self.dcn_weight = torch.nn.Parameter(torch.randn([dcn_out_channel, dcn_affine_out_channel, kernel_size, kernel_size]))
        ...
        self.affine = FullyConnectedLayer(w_dim, affine_out_channels, bias_init=1)
        memory_format = torch.channels_last if channels_last else torch.contiguous_format
        self.weight = torch.nn.Parameter(torch.randn([out_channels, affine_out_channels, kernel_size, kernel_size]).to(memory_format=memory_format))
        if self.cfg.use_noise:
            self.register_buffer('noise_const', torch.randn([resolution, resolution]))
            self.noise_strength = torch.nn.Parameter(torch.zeros([]))
        self.bias = torch.nn.Parameter(torch.zeros([out_channels]))

    def forward(self, x, w, noise_mode='random', fused_modconv=True, gain=1, canonical_feat=None):
        assert noise_mode in ['random', 'const', 'none']
        in_resolution = self.resolution // self.up
        # misc.assert_shape(x, [None, self.weight.shape[1], in_resolution, in_resolution])
        if self.cfg.canonical_cond == "concat":
            misc.assert_shape(x, [None, self.weight.shape[1] - self.cfg.canonical_cond_dim, in_resolution, in_resolution])
        else:
            misc.assert_shape(x, [None, self.weight.shape[1], in_resolution, in_resolution])

        styles = self.affine(w)

        offsets = offsets_mask = None
        # NOTE: after concat canonical_feat, the in_channels is changed, make dcn canonical aware
        if self.dcn and (not getattr(self.cfg, 'deform_dcn_syn_concat', True)):
            ...
            offset = modulated_conv2d(x=x, weight=dcn_weight.to(x.dtype), styles=dcn_styles, padding=self.padding)
        ...
        if self.cfg.canonical_cond == "concat":
            ...
            # NOTE: add project to change channel dim when needed
            if self.canonical_feat_proj is not None:
                canonical_feat = rearrange(canonical_feat, 'b c h w -> (b h w) c')
                canonical_feat = self.canonical_feat_proj(canonical_feat)
                canonical_feat = rearrange(canonical_feat, '(b h w) c -> b c h w', b=b_, h=h_, w=w_)
            x = torch.cat([x, canonical_feat], dim=1)

        # NOTE: after concat canonical_feat, the in_channels is changed, make dcn canonical aware
        if self.dcn and getattr(self.cfg, 'deform_dcn_syn_concat', True):
            # as w is fused with t before
            dcn_styles = self.dcn_affine(w)
            dcn_weight = self.dcn_weight * self.dcn_lr_mult
            dcn_bias = self.dcn_bias * self.dcn_lr_mult
        ...
        x = modulated_conv2d(x=x, weight=self.weight, styles=styles, noise=noise, up=self.up,
            ...
            offsets=offsets, offsets_mask=offsets_mask,
        )
...
class ToRGBLayer(torch.nn.Module):
    def __init__(self, in_channels, out_channels, w_dim, kernel_size=1, conv_clamp=None, channels_last=False, is_last=False, resolution=None, cfg={}):
        super().__init__()
        self.cfg = cfg
        self.resolution = resolution
        ...
        if self.is_last and (self.cfg.deform_init in ['zero', 'xavier']):
        ...
        if self.is_last:
            if self.cfg.deform_init == 'xavier':
                print("is_last, weight with xavier init")
                torch.nn.init.xavier_uniform_(self.weight, gain=1.0)
            elif self.cfg.deform_init == 'zero':
                print("is_last, weight with zero init")
                torch.nn.init.zeros_(self.weight)

        self.dcn = False
        self.dcn_lr_mult = self.cfg.dcn_lr_mult
        if self.cfg.deform_dcn and self.cfg.deform_dcn_torgb and (resolution >= self.cfg.deform_dcn_min_res and resolution <= self.cfg.deform_dcn_max_res) and (not self.is_last):
            print("torgb, resolution=%d, use dcn" % resolution)
            self.dcn = True
            dcn_out_channel = 3 * kernel_size ** 2
            self.dcn_affine = FullyConnectedLayer(w_dim, in_channels, bias_init=1)
            self.dcn_weight = torch.nn.Parameter(torch.randn([dcn_out_channel, in_channels, kernel_size, kernel_size]))
            self.dcn_scales = torch.nn.Parameter(torch.zeros([1, dcn_out_channel, 1, 1]))
            self.dcn_bias = torch.nn.Parameter(torch.zeros([1, dcn_out_channel, 1, 1]))
            self.padding = kernel_size // 2
...
class SynthesisBlock(torch.nn.Module):
        ...
        **layer_kwargs,                     # Arguments for SynthesisLayer.
        ...
        if is_last or architecture == 'skip':
            self.torgb = ToRGBLayer(out_channels, img_channels, w_dim=w_dim,
                ...
                is_last=is_last, resolution=resolution, cfg=cfg)
            self.num_torgb += 1
    ...
    def forward(self, x, img, ws, motion_v=None, force_fp32=False, fused_modconv=None, canonical_feat=None, **layer_kwargs):
        ...
        # Main layers.
        if self.in_channels == 0:
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, canonical_feat=canonical_feat, **layer_kwargs)
        elif self.architecture == 'resnet':
            y = self.skip(x, gain=np.sqrt(0.5))
            x = self.conv0(x, next(w_iter), fused_modconv=fused_modconv, canonical_feat=canonical_feat, **layer_kwargs)
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, canonical_feat=canonical_feat, gain=np.sqrt(0.5), **layer_kwargs)
            x = y.add_(x)
        else:
            conv0_w = next(w_iter)
            x = self.conv0(x, conv0_w, fused_modconv=fused_modconv, canonical_feat=canonical_feat, **layer_kwargs)
            x = self.conv1(x, next(w_iter), fused_modconv=fused_modconv, canonical_feat=canonical_feat, **layer_kwargs)
        ...
        if self.is_last or self.architecture == 'skip':
            ...
            if self.is_last and (self.cfg.deform_init in ['zero', 'xavier']):
                # split y to 2 parts in channel
                assert y.shape[1] == 2 * self.img_channels
                assert img is not None
                wight_final = y[:, :self.img_channels, :, :]
                bias_final = y[:, self.img_channels:, :, :]
                img = img * wight_final + bias_final
            else:
...
@persistence.persistent_class
class StyleGAN2SynthesisNetwork(torch.nn.Module):
    def __init__(self,
        w_dim,                      # Intermediate latent (W) dimensionality.
    ...
    def forward(self, ws, t=None, c=None, motion_z=None, motion_v=None, feat_canonical_dict=None, **block_kwargs):
        ...
        if self.cfg.canonical_cond == "none":
            ...
            assert len(feat_canonical_dict) == 1
            assert self.cfg.canonical_feat in feat_canonical_dict
            canonical_feat = feat_canonical_dict[self.cfg.canonical_feat]  # (b, dim, 256, 256)
            canonical_feat = canonical_feat.repeat_interleave(t.shape[1], dim=0)  # (bt, dim, 256, 256)
        ...
        for res, cur_ws in zip(self.block_resolutions, block_ws):
            ...
            x, img = block(x, img, cur_ws, motion_v=motion_v, canonical_feat=canonical_feat, **block_kwargs)
...
class Generator(torch.nn.Module):
    def __init__(self,
        ...
        self.img_resolution = img_resolution
        self.img_channels = img_channels
        self.synthesis = StyleGAN2SynthesisNetwork(w_dim=w_dim, img_resolution=img_resolution, img_channels=img_channels, cfg=cfg, **synthesis_kwargs)
