...
def random_frame_sampling(cfg: Dict, total_video_len: int, use_fractional_t: bool=False) -> np.ndarray:
    ...
    max_time_diff = min(total_video_len - 1, cfg.get('max_dist', float('inf')))  #  cfg.max_dist=32

    if type(cfg.get('total_dists')) in (list, tuple):
        # [1, 2, 4, 8, 16, 32, 64, 128, 256, 512, 1024, 2048] -> [2, 4, 8, 16, 32]
    ...
    new_frames_idx = frames_idx

    # new_frames_idx = [frames_idx[0]]

    # max_frame_dist = cfg.get('max_frame_dist', 30)
    # for idx in range(len(frames_idx)-1):
    #     diff = frames_idx[idx+1] - frames_idx[idx]
    #     diff = min(diff, max_frame_dist)
    #     new_frames_idx.append(new_frames_idx[-1]+diff)

    return np.array(new_frames_idx)
