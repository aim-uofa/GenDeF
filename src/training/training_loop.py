...
def training_loop(
    ...
    # NOTE: including depth changes input channels, luckily all these models use the same input transform
    assert cfg.model.discriminator.depth in ['none', 'dpt_beit_large_512', 'dpt_swin2_large_384', 'dpt_swin2_tiny_256']
    depth_model = None
    depth_net_w = depth_net_h = None
    img_channels_d = 3
    if cfg.model.discriminator.depth != 'none':
        from src.midas.model_loader import load_model
        print(f"using depth model: {cfg.model.discriminator.depth}")
        # create depth model
        depth_model, depth_transform, depth_net_w, depth_net_h = load_model(
            device=device, 
            model_path=cfg.model.discriminator.depth_ckpt, 
            model_type=cfg.model.discriminator.depth, 
            optimize=False, height=None, square=False,
        )
        # freeze depth model weight
        for n, p in depth_model.named_parameters():
            p.requires_grad = False

        # concat depth to image
        img_channels_d += 1

    common_kwargs_g = dict(c_dim=training_set.label_dim, img_resolution=training_set.resolution, img_channels=training_set.num_channels)
    ...
    common_kwargs_d = dict(c_dim=training_set.label_dim, img_resolution=training_set.resolution, img_channels=img_channels_d)
    ...
    if (resume_pkl is not None):
        ...
        try:
            # manually load
            model_state_dicts_org = torch.load(resume_pkl, map_location=torch.device('cpu'))

            if cfg.model.generator.deform_weight_copy: 
                G_params_dict = dict(G.named_parameters())
                model_state_dicts = copy.deepcopy(model_state_dicts_org)
                for k, v in model_state_dicts_org['G'].items():
                    if k.startswith('synthesis.') or k.startswith('mapping.'):
                        # if '_ema' in k or 'L14' in k:
                        if 'L14' in k:
                            print(f"ignore {k}")
                            continue

                        if k.startswith('synthesis.'):
                            k_new = k.replace('synthesis.', 'synthesis_deform.')
                        elif k.startswith('mapping.'):
                            k_new = k.replace('mapping.', 'mapping_deform.')

                        if ('down_filter' in k) or ('up_filter' in k) or ('magnitude_ema' in k):
                            # not in G.named_parameters(), just copy
                            print(f"copy {k} to {k_new}")
                            model_state_dicts['G'][k_new] = v
                            continue

                        if k_new not in G_params_dict:
                            print(f"not found: {k_new}, donot copy")
                            continue

                        target_shape = G_params_dict[k_new].shape
                        if target_shape == v.shape:
                            print(f"copy {k} to {k_new}")
                            model_state_dicts['G'][k_new] = v
                        else:
                            print("mismatch:", k_new, target_shape, v.shape)
            else:
                model_state_dicts = model_state_dicts_org

            missing_keys, unexpected_keys = G.load_state_dict(model_state_dicts['G'], strict=False)
            G_ema = copy.deepcopy(G).eval()
            if rank == 0:
                print('============================================')
            ...
            b4_weight = model_state_dicts['D']['b4.out.weight']
            b4_bias = model_state_dicts['D']['b4.out.bias']
            model_state_dicts['D'].pop('b4.out.weight')
            model_state_dicts['D'].pop('b4.out.bias')
            if cfg.model.discriminator.depth != 'none':
                b256_weight = model_state_dicts['D']['b256.fromrgb.weight']
                model_state_dicts['D'].pop('b256.fromrgb.weight')
            missing_keys, unexpected_keys = D.load_state_dict(model_state_dicts['D'], strict=False)
            if cfg.model.discriminator.depth != 'none':
                b256_weight_new = D.b256.fromrgb.weight
                b256_weight_mix = torch.cat((b256_weight, b256_weight_new[:, 3:4, ...].to(b256_weight.device)), dim=1)
                D.b256.fromrgb.weight.data.copy_(b256_weight_mix)
                missing_keys.pop(missing_keys.index('b256.fromrgb.weight'))
            # manually load b4 out fc (broadcasting tensor maybe)
    ...
    if rank == 0 and not resume_whole_state:
        ...
        if cfg.model.discriminator.depth != 'none':
            img = torch.cat((img, img[:, 0:1, ...]), dim=1)
    ...
    if cfg.model.generator.with_canonical:
        modules.append(('G_mapping_deform', G.mapping_deform))
        modules.append(('G_synthesis_deform', G.synthesis_deform))
    ...
    loss = dnnlib.util.construct_class_by_name(
        device=device, depth_model=depth_model, depth_net_h=depth_net_h, depth_net_w=depth_net_w,
    ...
    #print('resume optimization:', resume_whole_state, flush=True)
    #if resume_whole_state:
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_z_addition, phase_gen_c, phase_gen_t in zip(phases, all_gen_z, all_gen_z_addition, all_gen_c, all_gen_t):
            ...
            # TODO: optionally freeze canonical layers
            # # set frozen layer not to require grad
            # if 'G' in phase.name:
            #     for i in range(cfg.model.generator.freezesyn):
            #         for param in getattr(phase.module.synthesis, names[-1-i]).parameters():
            #             param.requires_grad_(False)

            if cfg.model.generator.freeze_canonical:
                if 'G' in phase.name:
                    for p in getattr(phase.module, 'mapping').parameters():
                        p.requires_grad_(False)
                    for p in getattr(phase.module, 'synthesis').parameters():
                        p.requires_grad_(False)
            ...
            for metric in metrics:
                ...
                if metric == 'fvd2048_16f':
                    ...
                    if fvd16_value < fvd16_best:
                        ...
                        save_ckpt = True
                        save_json = True
                if metric == 'fvd2048_128f':
                    fvd128_value = result_dict['results']['fvd2048_128f']
                    if fvd128_value < fvd128_best:
                        fvd128_best = fvd128_value
                        save_json = True
        ...
        if save_json:
            if rank == 0:
                jsonl_line = json.dumps(dict(fvd16_best=fvd16_best, fvd128_best=fvd128_best, fid_best=fid_best, timestamp=time.time()))
                with open(os.path.join(run_dir, f'metric-best.jsonl'), 'at') as f:
                    f.write(jsonl_line + '\n')
