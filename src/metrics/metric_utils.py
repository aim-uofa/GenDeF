...
class MetricOptions:
    def __init__(self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1, rank=0, device=None,
                       progress=None, interpw=False, cache=True, gen_dataset_kwargs={}, generator_as_dataset=False):
...
cache_path = '/input/ceyuan/cache'
# cache_path = "/nas2/wwen/weights/StyleSV/cache"
def get_feature_detector(url, device=torch.device('cpu'), num_gpus=1, rank=0, verbose=False):
    ...
    if key not in _feature_detector_cache:
        ...
        try:
            ...
            if fn.endswith('?dl=1'):
                fn = fn[:-5]
            cy_url = f"{cache_path}/{fn}" 
            with dnnlib.util.open_url(cy_url, verbose=(verbose and is_leader)) as f:
                if urlparse(cy_url).path.endswith('.pkl'):
...
def compute_feature_stats_for_dataset(
    ...
    feature_stats_cls=FeatureStats, cache_suffix='', **stats_kwargs):
    ...
    if opts.cache:
        ...
        cache_tag = f'{dataset.name}-{get_feature_detector_name(detector_url)}{cache_suffix}'
...
def compute_feature_stats_for_generator(
    ...
    def run_generator(z, c, t, z_addition=None):
        if z_addition is not None:
            assert len(opts.G_kwargs) == 0
            w = G.mapping(z, c=c).unsqueeze(1).repeat(1, t.shape[1], 1, 1)
            w_addition = G.mapping(z_addition, c=c).unsqueeze(1).repeat(1, t.shape[1], 1, 1)
            k = 1 - t / 4096
            k = k.view(k.shape[0], k.shape[1], 1, 1)
            w = k * w + (1-k) * w_addition
            w = w.view(-1, w.shape[2], w.shape[3])
            img = G.synthesis(ws=w, c=c, t=t)
        else:
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            z_addition = torch.randn([batch_gen, G.z_dim], device=opts.device)
            ...
            images.append(run_generator(z, c, t) if not opts.interpw else run_generator(z,c,t,z_addition))
