...
from src.training.logging_ours import generate_videos, save_video_frames_as_mp4, save_video_frames_as_frames_parallel
from src.flow_vis.flow_vis import flow_to_color
...
@click.option('--save_only_rgb', help='Should we save as independent frames or mp4?', type=bool, default=False, metavar='BOOL')
...
@click.option('--slowmo_coef', help='Increase this value if you want to produce slow-motion videos.', default=1, type=float, metavar='INT')
def generate(
    ctx: click.Context,
    network_pkl: str,
    networks_dir: str,
    ...
    outdir = os.path.join(outdir, f"{video_len}_slow{slowmo_coef}")
    if as_grids:
        outdir = outdir + '_grids'
    
    if save_only_rgb:
        outdir = outdir + '_onlyrgb'
    ...
    for batch_idx in tqdm(range((num_videos + batch_size - 1) // batch_size), desc='Generating videos'):
        ...
        # False
        if interp:
            videos, videos_canonical, videos_deformation, videos_deformation_offset = generate_videos(
                G, z, c, ts[:curr_batch_size], z_addition, motion_z=motion_z, noise_mode=noise_mode,
                truncation_psi=truncation_psi, as_grids=as_grids, batch_size_num_frames=video_len, return_aux=True)
        else:
            videos, videos_canonical, videos_deformation, videos_deformation_offset = generate_videos(
        ...
        if as_grids:
            videos = [videos]
            videos_canonical = [videos_canonical]
            videos_deformation = [videos_deformation]
            videos_deformation_offset = [videos_deformation_offset]

        for video_idx, (video, canonical, deformation, deformation_offset) in enumerate(zip(videos, videos_canonical, videos_deformation, videos_deformation_offset)):
            # we save both video and frames
            save_path = os.path.join(outdir, f'video_{batch_idx * batch_size + video_idx:06d}.mp4')
            save_video_frames_as_mp4(video, fps, save_path)

            save_dir = os.path.join(outdir, f'video_{batch_idx * batch_size + video_idx:06d}')
            # save video frames in image dir
            ...
            # save canonical in out dir
            canonical = (canonical * 255).permute(0, 2, 3, 1).to(torch.uint8).numpy().squeeze(0) # [h, w, c]
            Image.fromarray(canonical).save(os.path.join(save_dir, f'canonical_{batch_idx * batch_size + video_idx:06d}.png'), q=95)

            if not save_only_rgb:
                # save deformation as npy file in out dir
                deformation = deformation.permute(0, 2, 3, 1).cpu().numpy() # (t, h, w, 2)
                np.save(os.path.join(save_dir, f'{batch_idx * batch_size + video_idx:06d}_deform.npy'), deformation)
                # os.makedirs(save_dir + "_deform", exist_ok=True)
                # for frame_idx in range(len(deformation)):
                #     TVF.to_pil_image(flow_to_color(deformation[frame_idx])).save(os.path.join(save_dir + "_deform", f'{frame_idx:06d}_deform.png'))

                # save deformation offset
                deformation_offset = deformation_offset.permute(0, 2, 3, 1).cpu().numpy() # (t, h, w, 2)
                os.makedirs(save_dir + "_deform_offset", exist_ok=True)
                for frame_idx in range(len(deformation_offset)):
                    TVF.to_pil_image(flow_to_color(deformation_offset[frame_idx])).save(os.path.join(save_dir + "_deform_offset", f'{frame_idx:06d}_deform_offset.png'))

                # obtain flow from deformation
                # flow_x = deformation[:, :, :, 0] - deformation_offset[:, :, :, 0]
                flow_x = deformation[1:, :, :, 0:1] - deformation[:-1, :, :, 0:1]
                flow_y = deformation[1:, :, :, 1:2] - deformation[:-1, :, :, 1:2]
                flow = np.concatenate([flow_x, flow_y], axis=-1)
                os.makedirs(save_dir + "_flow", exist_ok=True)
                for frame_idx in range(len(flow)):
                    TVF.to_pil_image(flow_to_color(flow[frame_idx])).save(os.path.join(save_dir + "_flow", f'{frame_idx:06d}_flow.png'))
